    ...
    (function(global, pool2, math) {
      ...
      function autoseed() {
        try {
          ...
          return [+/* @__PURE__ */ new Date(), global, plugins, global.screen, tostring(pool2)];
        }
      }
      function tostring(a) {
...
function logSoftmax_(logits, axis = -1) {
  ...
  const customOp = customGrad((logits2, save) => {
    ...
    const value = sub(cast(shifted, "float32"), log2(sum(exp(shifted), axis, keepDims)));
    save([value]);
...
function logLoss_(labels, predictions, weights, epsilon = 1e-7, reduction = Reduction.SUM_BY_NONZERO_WEIGHTS) {
  ...
  const l1 = neg(mul($labels, log2(add($predictions, epsilonScalar))));
  const l2 = mul(sub(one, $labels), log2(add(sub(one, $predictions), epsilonScalar)));
...
@tensorflow/tfjs-core/dist/index.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-JV5TYECV.js.map
